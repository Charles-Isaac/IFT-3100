...
void Application::setup()
{
	...
	gui->onToggleEvent(this, &Application::onToggleEvent);
	gui->onSliderEvent(this, &Application::onSliderEvent);
	gui->onTextInputEvent(this, &Application::onTextInputEvent);
	gui->on2dPadEvent(this, &Application::on2dPadEvent);
	gui->onDropdownEvent(this, &Application::onDropdownEvent);
	gui->onColorPickerEvent(this, &Application::onColorPickerEvent);
	gui->onMatrixEvent(this, &Application::onMatrixEvent);
...
void Application::onTextInputEvent(ofxDatGuiTextInputEvent e)
{
	cout << "onTextInputEvent: " << e.target->getLabel() << " " << e.target->getText() << endl;
}

void Application::on2dPadEvent(ofxDatGui2dPadEvent e)
{
	cout << "on2dPadEvent: " << e.target->getLabel() << " " << e.x << ":" << e.y << endl;
}

void Application::onDropdownEvent(ofxDatGuiDropdownEvent e)
{
	cout << "onDropdownEvent: " << e.target->getLabel() << " Selected" << endl;
}

void Application::onColorPickerEvent(ofxDatGuiColorPickerEvent e)
{
	cout << "onColorPickerEvent: " << e.target->getLabel() << " " << e.target->getColor() << endl;
	ofSetBackgroundColor(e.color);
}

void Application::onMatrixEvent(ofxDatGuiMatrixEvent e)
{
